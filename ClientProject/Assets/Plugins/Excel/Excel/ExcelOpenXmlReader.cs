        ...
        private void ReadSheetGlobals(XlsxWorksheet sheet)
        {
            ...
            if (this._sheetStream != null)
            {
                ...
                while (this._xmlReader.Read())
                {
                    ...
                    {
                        string reference = this._xmlReader.GetAttribute("r");
                        if (reference != null)
                        {
                            int[] numArray = ReferenceHelper.ReferenceToColumnAndRow(reference);
                            if (numArray[1] > num3)
                            {
                                num3 = numArray[1];
                            }
        ...
        private bool ReadSheetRow(XlsxWorksheet sheet)
        {
            ...
            while (this._xmlReader.Read())
            {
                ...
                if ((this._xmlReader.NodeType == XmlNodeType.Text) && flag)
                {
                    ...
                    if (double.TryParse(obj2.ToString(), any, invariantCulture, out num4))
                    {
                        obj2 = num4;
                    }
