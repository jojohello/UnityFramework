namespace Excel
{
    ...
    public class ExcelBinaryReader : IExcelDataReader, IDataReader, IDisposable, IDataRecord
    {
        ...
        {
            double num;
            LogManager.Log<ExcelBinaryReader>(this).Debug("pushCellValue {0}", new object[] { cell.ID });
            switch (cell.ID)
            {
                case BIFFRECORDTYPE.RK:
                    ...
                    LogManager.Log<ExcelBinaryReader>(this).Debug("VALUE: {0}", new object[] { num });
                    return;
                ...
                case BIFFRECORDTYPE.RSTRING:
                    ...
                    LogManager.Log<ExcelBinaryReader>(this).Debug("VALUE: {0}", new object[] { this.m_cellsValues[cell.ColumnIndex] });
                    return;
                ...
                case BIFFRECORDTYPE.MULRK:
                {
                    ...
                    for (ushort i = cell.ColumnIndex; i <= cell2.LastColumnIndex; i = (ushort) (i + 1))
                    {
                        ...
                        LogManager.Log<ExcelBinaryReader>(this).Debug("VALUE[{1}]: {0}", new object[] { num, i });
                    ...
                    switch (iD)
                    {
                        ...
                        case BIFFRECORDTYPE.BOUNDSHEET:
                        {
                            ...
                            if (refSheet.Type == XlsBiffBoundSheet.SheetType.Worksheet)
                            {
                                ...
                                LogManager.Log<ExcelBinaryReader>(this).Debug("BOUNDSHEET IsV8={0}", new object[] { refSheet.IsV8 });
        ...
        private bool readWorkSheetGlobals(XlsWorksheet sheet, out XlsBiffIndex idx, out XlsBiffRow row)
        {
            ...
            {
                idx.IsV8 = this.isV8();
                LogManager.Log<ExcelBinaryReader>(this).Debug("INDEX IsV8={0}", new object[] { idx.IsV8 });
            }
            ...
            while (row2 == null)
            {
                ...
                LogManager.Log<ExcelBinaryReader>(this).Debug("finding rowRecord offset {0}, rec: {1}", new object[] { record3.Offset, record3.ID });
            ...
            if (row2 != null)
            {
                LogManager.Log<ExcelBinaryReader>(this).Debug("Got row {0}, rec: id={1},rowindex={2}, rowColumnStart={3}, rowColumnEnd={4}", new object[] { row2.Offset, row2.ID, row2.RowIndex, row2.FirstDefinedColumn, row2.LastDefinedColumn });
            }
            row = row2;
            if (dimensions != null)
            {
                dimensions.IsV8 = this.isV8();
                LogManager.Log<ExcelBinaryReader>(this).Debug("dims IsV8={0}", new object[] { dimensions.IsV8 });
