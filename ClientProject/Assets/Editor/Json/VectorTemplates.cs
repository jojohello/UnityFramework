...
public static partial class JSONTemplates
{
    ...
    public static RectOffset ToRectOffset(JSONObject obj)
    {
        ...
        for (int i = 0; i < obj.Count; i++)
        {
            switch ((string)obj.keys[i])
            {
                case "bottom": r.bottom = (int)obj[i].number; break;
                case "left": r.left = (int)obj[i].number; break;
                case "right": r.right = (int)obj[i].number; break;
                case "top": r.top = (int)obj[i].number; break;
            }
        }
        return r;
    }
